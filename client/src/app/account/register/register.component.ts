...
export class RegisterComponent implements OnInit {
  ...
  showForm(event?: any) {
    ...
    if (!this.type) { this.type = event.target.value; }

    // If the user type is patient, disable the form control doctorField. Else, enable it.
    if (this.type === 'patient') { this.registerForm.controls.doctorField.disable(); }
    else { this.registerForm.controls.doctorField.enable(); }
  }

  // Change the user type from doctor to patient, Or from patient to doctor. Based on the current type.
  changeUserType() {
    this.type = this.type === 'doctor' ? 'patient' : 'doctor';
    this.showForm();
  }
